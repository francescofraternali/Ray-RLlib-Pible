...
def event_func(t_now, next_wake_up_time, events): # check how many events are on this laps of time
        event = 0
        for check in events:
            if t_now <= check and check < next_wake_up_time:
                event += 1
                #vents.remove(check)
            #print(t_now, check, next_wake_up_time, event)
        return event, events
...
def events():
    ...
    events.append(datetime.datetime.strptime('01/01/17 20:05:00', '%m/%d/%y %H:%M:%S'))
    events.append(datetime.datetime.strptime('01/01/17 22:05:00', '%m/%d/%y %H:%M:%S'))
    return events
...
def plot_hist(Time, Light, PIR_OnOff, State_Trans, Reward, Perf, SC_Volt, SC_Norm, PIR, episode, tot_rew):
    ...
    plt.title(('Simulation Results for PIR Event Detection. Tot reward: {0}').format(tot_rew))
