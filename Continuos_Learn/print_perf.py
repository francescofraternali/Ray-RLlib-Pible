...
folders = ["12-hours", "1-day", "1-week", "2-week"]

for path in folders:
    num_death = 0
    percent_list = []
    for line in open(path +'/results.json', 'r'):
        data = json.loads(line)
        percent_list.append(data["percent"])
        death = int(data["Death_times"])
        if death > 0:
            num_death += 1

    print("Avg_percent_list ", path, sum(percent_list)/len(percent_list), "Death times", num_death)
exit()
...
def find_agent_saved():
    ...
    for line in open(path + "/ray_results/" + folder + "/result.json", 'r'):
        ...
        if dict['episode_reward_mean'] >= max_mean and count > 49:
