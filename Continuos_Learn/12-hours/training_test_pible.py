...
def check_perf_results(folder, iteration, start, end, sc_volt_start_test):
    Agnt = "PPO"
    ...
    while True:
        ...
        if stop >= repeat*(0.5):
            ...
            break
    print("Energy Prod: ", energy_prod_tot/(repeat*0.5), "Energy Used: ", energy_used_tot/(repeat*0.5) )

    print("Tot reward: ", tot_rew)
    print_start = start
    print_end = end
    print("Tot events averaged per day: ", int(res["Tot_events"])/(repeat*0.5))
    ...
    Pible_func.write_results(tot_rew, start, end, percent, 0.5,
        energy_prod_tot/(repeat*0.5), energy_used_tot/(repeat*0.5), res["events_detect"],
        int(res["Tot_events"]), death_days_tot, death_min_tot, volt_diff)
    print("Percent detected: ", percent)


    #print(sc_volt_start_test)
    #sleep(5)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        config={
            ...
            "batch_mode": "complete_episodes",
            "lr" : grid_search([1e-4]),
            "num_workers": int(spl[0])-1,  # parallelism
            "env_config": {
                "train/test": "train",
        ...
        folder, iteration = Pible_func.find_agent_saved()
        ...
        sleep(10)
        resume, sc_volt_start_test = check_perf_results(folder, iteration, start_test, end_test, sc_volt_start_test)
